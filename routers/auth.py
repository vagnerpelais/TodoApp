...
SECRET_KEY = 'a5lbkOX8ipdINqZYA8uDtYmmvivLdnsX'
ALGORITHM = 'HS256'
...
@router.post('/token')
async def login_for_access_token(response: Response, form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_database)):
...
async def register_user(request: Request, email: str = Form(...), username: str = Form(...),
                        ...
                        password2: str = Form(...), db: Session = Depends(get_database)):
